...
//#endregion
//#region server/middleware/00-legacy-ios.ts
function isLegacyIOS(ua) {
...
async function legacyIosMiddleware(event, next) {
	if ((event.req.method ?? "GET").toUpperCase() !== "GET") return next();
	const path = event.url.pathname;
	if (isLegacyIOS(event.req.headers.get("user-agent") ?? "") && (path === "/" || path === "")) return new Response(null, {
		status: 302,
...
var globalMiddleware = [toEventHandler(legacyIosMiddleware), toEventHandler(grokPwaMiddleware)].filter(Boolean);
//#endregion
